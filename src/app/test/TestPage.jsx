...
export default function TestPage() {
  ...
  /* ── Overall test timer ── */
  useEffect(() => {
    if (!config || isFinished) return // ✅ Also stop when finished
  ...
  }, [overallTime, config, isFinished]) // ✅ Added isFinished
  ...
  /* ── Track visited ── */
  useEffect(() => {
    if (!filteredQuestions.length) return // ✅ Fixed syntax error
      ...
      <div className={`${styles.testContainer} ${styles.fadeIn}`}>
        <div className={styles.timerRow}>
          <div className={styles.overallTimer}>
            Time Left: {Math.floor(overallTime / 60)}:{String(overallTime % 60).padStart(2, "0")}
          </div>
          <Timer key={currentPage} duration={config.duration} onTimeUp={handleTimeUp} />
        </div>
