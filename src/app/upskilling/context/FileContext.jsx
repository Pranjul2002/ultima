  ...
  const uploadSource = useCallback(
    async (localId, file) => {
      ...
      try {
        ...
        pollSourceStatus(localId, backendId);
      } catch (error) {
        updateSource(localId, {
          status: "failed",
    ...
    (incomingFiles) => {
      ...
      if (freshSources.length > 0) {
        ...
        freshSources.forEach((source) => {
          uploadSource(source.localId, source.file);
        });
      }
    },
    [uploadSource]
  ...
  const askQuestion = useCallback(
    async (sourceId, question) => {
      ...
      if (source.status !== "ready") {
        throw new Error("This source is still being processed.");
      }

      if (!source.backendId) {
        throw new Error("This source is missing a backend id.");
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data?.message || "Unable to get response from AI.");
        }
